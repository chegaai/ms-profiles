...
helm upgrade --install --atomic ms-profiles-${NAMESPACE} \
  ...
  --set "environment.MS_PROFILES_AZURE_STORAGE_ACCOUNT_NAME=$MS_PROFILES_AZURE_STORAGE_ACCOUNT_NAME" \
  --set "environment.MS_PROFILES_AZURE_STORAGE_ACCOUNT_ACCESS_KEY=$MS_PROFILES_AZURE_STORAGE_ACCOUNT_ACCESS_KEY" \
  --namespace $NAMESPACE \
